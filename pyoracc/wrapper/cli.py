...
def main(input_path, atf_type, verbose):
    ...
    if os.path.isdir(input_path):
        failures = 0
        successes = 0
        with click.progressbar(os.listdir(input_path), label='Info: Checking the files') as bar:
            for f in bar:
                ...
                try:
                    ...
                    failures += 1
                    click.echo("Info: Failed with message: {0} in {1}".format(e, pathname))
        click.echo("Failed with {0} out of {1} ({2}%)".format(failures, failures + successes, failures * 100.0 / (failures + successes)))
    else:
        try:
            ...
            click.echo('Info: Correctly parsed {0}.'.format(input_path))
                ...
                UnicodeDecodeError) as e:
            click.echo("Info: Failed with message: {0} in {1}".format(e, input_path))